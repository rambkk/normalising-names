...
FROM table1 WHERE REGEXP_REPLACE(HEX(column1),"([[:xdigit:]]{2,2})",'$1 ') REGEXP 
...
SELECT id,column1,REGEXP_REPLACE(HEX(column1),"([[:xdigit:]]{2,2})",'$1 ') AS hex 
FROM table1 WHERE REGEXP_REPLACE(HEX(column1),"([[:xdigit:]]{2,2})",'$1 ') REGEXP '0[:xdigit:]|1[:xdigit:]|7F|C2 AD|E2 80 8B|E2 80 8C|E2 80 8D|E3 80 80|EF BB BF'

SELECT id,column1,REGEXP_REPLACE(HEX(column1),"([0-9A-F]{2,2})",'$1 ') AS hex 
FROM table1 WHERE REGEXP_REPLACE(HEX(column1),"([0-9A-F]{2,2})",'$1 ') REGEXP '0[0-9A-F]|1[0-9A-F]|7F|C2 AD|E2 80 8B|E2 80 8C|E2 80 8D|E3 80 80|EF BB BF' 

SELECT id,column1,REGEXP_REPLACE(HEX(column1),"([0-9A-F]{2,2})",'$1 ') AS hex 
FROM table1 WHERE REGEXP_REPLACE(HEX(column1),"([0-9A-F]{2,2})",'$1 ') REGEXP '[01][0-9A-F]|7F|C2 AD|E2 80 8[BCD]|E3 80 80|EF BB BF' 
